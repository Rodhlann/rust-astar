...
const MATRIX_W: usize = 20;
const MATRIX_H: usize = 20;

fn a_star() -> [[i32; MATRIX_W]; MATRIX_H] {
    let start = Point { x: 8, y: 3 };
    let end = Point { x: 6, y: 3 };
    let mut walls: Vec<Point> = Vec::new();
    walls.push(Point { x: 4, y: 1 });
    walls.push(Point { x: 4, y: 2 });
    walls.push(Point { x: 4, y: 4 });
    walls.push(Point { x: 4, y: 5 });
    walls.push(Point { x: 5, y: 1 });
    walls.push(Point { x: 5, y: 5 });
    walls.push(Point { x: 6, y: 1 });
    walls.push(Point { x: 6, y: 5 });
    walls.push(Point { x: 7, y: 1 });
    walls.push(Point { x: 7, y: 2 });
    walls.push(Point { x: 7, y: 3 });
    walls.push(Point { x: 7, y: 4 });
    walls.push(Point { x: 7, y: 5 });
    ...
    let best: Node = loop {
        ...
        for dir in Dirs::iter() {
            ...
            if next_pos.x >= MATRIX_W as i32
                || next_pos.x < 0 
                || next_pos.y >= MATRIX_H as i32
                || next_pos.y < 0
    ...
    let mut matrix = [[0; MATRIX_W]; MATRIX_H];
...
#[test]
fn a_star_test() {
    // let result = a_star(width, height, start, end);
}
